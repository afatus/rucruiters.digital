/*
  ...
  1. Security Updates
    - Update jobs table RLS policy to allow it_admin and super_admin to see all jobs
    - Update interviews table RLS policies to allow it_admin and super_admin to see all interviews
    - Ensure proper tenant isolation for non-admin users

  2. Changes Made
    - Modified jobs SELECT policy to include it_admin role
    - Replaced overly permissive interviews SELECT policy with proper tenant-aware policy
    - Added explicit admin access for both tables
*/
...
CREATE POLICY "Users can view jobs based on role and tenant"
  ...
  USING (
    ...
    (((jwt() -> 'user_metadata'::text) ->> 'role'::text) = 'super_admin'::text) OR
    (((jwt() -> 'user_metadata'::text) ->> 'role'::text) = 'it_admin'::text) OR
...
BEGIN
  ...
  IF ((jwt() -> 'user_metadata'::text) ->> 'role'::text) IN ('super_admin', 'it_admin') THEN
    RETURN null;
  END IF;
  ...
  USING (
    ...
    (EXISTS (
      ...
      WHERE ((i.id = video_responses.interview_id) AND 
             ((j.created_by = uid()) OR 
              (j.hiring_manager_id = uid()) OR 
              (j.line_manager_id = uid()) OR 
              (((jwt() -> 'user_metadata'::text) ->> 'role'::text) = 'hr_operations'::text)))
    ))
  );
  ...
  USING (
    ...
    (EXISTS (
      ...
      WHERE ((jobs.id = interview_questions.job_id) AND 
             ((jobs.created_by = uid()) OR 
              (jobs.hiring_manager_id = uid()) OR 
              (jobs.line_manager_id = uid()) OR 
              (((jwt() -> 'user_metadata'::text) ->> 'role'::text) = 'hr_operations'::text)))
    ))
  );
