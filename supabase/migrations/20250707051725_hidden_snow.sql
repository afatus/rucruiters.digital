/*
  ...
  1. Changes
    - Drop all existing RLS policies on the jobs table
    - Create new, clearer policies for different operations
    - Ensure proper access for all roles, especially recruiters
    - Fix issue with jobs not appearing in the menu

  2. New Policies
    - jobs_it_admin_super_admin_full_access: Full access for admins
    - jobs_tenant_based_access: Tenant-based SELECT access
    - jobs_stakeholders_manage: Role-based access for job stakeholders
    - Only logged-in users can insert jobs
    - Recruiters and HR can create jobs
*/
...
-- 4. Only logged-in users can insert jobs
CREATE POLICY "Only logged-in users can insert jobs" ON jobs
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = created_by);

-- 5. Recruiters and HR can create jobs
CREATE POLICY "Recruiters and HR can create jobs" ON jobs
  FOR INSERT
  TO authenticated
  WITH CHECK (
    ((auth.jwt() ->> 'user_metadata')::json ->> 'role') = ANY (ARRAY['recruiter', 'hr_operations', 'it_admin'])
  );